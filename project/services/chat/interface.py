...
class ChatInterface:
    ...
    def chat(self, message, history, session_id: str):
        """Generator that streams Gradio chat message dicts."""
        if not self.rag_system.agent_graph:
            yield "⚠️ System not initialized!"
            return

        config        = self.rag_system.get_config(session_id)
    ...
    async def achat(self, message, history, session_id: str):
        """Versión async del generador para FastAPI (Evita bloquear el event loop)."""
        if not self.rag_system.agent_graph:
            yield "⚠️ System not initialized!"
            return
