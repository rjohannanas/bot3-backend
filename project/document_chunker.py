...
class DocumentChunker:
    ...
    def create_chunks_single(self, md_path, source_url=None, extra_metadata=None):
        ...
        all_parent_chunks, all_child_chunks = [], []
        self.__create_child_chunks(all_parent_chunks, all_child_chunks, cleaned_parents, doc_path, source_url, extra_metadata)
        return all_parent_chunks, all_child_chunks
    ...
    def __create_child_chunks(self, all_parent_pairs, all_child_chunks, parent_chunks, doc_path, source_url=None, extra_metadata=None):
        for i, p_chunk in enumerate(parent_chunks):
            parent_id = f"{doc_path.stem}_parent_{i}"
            source_val = source_url if source_url else f"{doc_path.stem}.pdf"
            base_meta = {"source": source_val, "parent_id": parent_id}
            if extra_metadata and isinstance(extra_metadata, dict):
                base_meta.update(extra_metadata)
            p_chunk.metadata.update(base_meta)
